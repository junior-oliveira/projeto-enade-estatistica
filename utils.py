...
def renomearColunas(ano):
    ...
    base_de_dados = pd.read_csv("base_de_dados/"+conf.BASES_DE_DADOS[ano], delimiter=';')

    # Altera os cabeçalhos do DataFrame para maiúsculas
    base_de_dados.columns = map(str.upper, base_de_dados.columns)

    df_novos_rotulos = pd.read_excel('rotulos_colunas/rotulos_colunas_todas_bases.xlsx')
